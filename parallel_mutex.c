...
int keys[NUM_KEYS];
pthread_mutex_t *lock_table[NUM_BUCKETS];
...
int main(int argc, char **argv) {
    ...
    for (i = 0; i < NUM_BUCKETS; i++) {
        lock_table[i] = (pthread_mutex_t*) malloc(sizeof(pthread_mutex_t));
        if (!lock_table[i]) panic("No memory to allocate lock_table!");
